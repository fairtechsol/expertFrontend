...
import { rates } from "../../../utils/InputKeys/Bookmaker/Rates";
...
const NewEditBookmaker = (props: any) => {
        ...
        try {
            ...
            if ((!match?.rateThan100) || match?.rateThan100) {
                ...
                setLocalQuickBookmaker((prev: any) => {
                    let updatedTeams = prev.teams?.map((item: any) => {

                        const backKey = (Number(value) + incGap)
                        const layKey = Number(value)
                        return item.id === name
    ...
    // console.log("localQuickBookmaker :", localQuickBookmaker)
    return (
        <>
            <Box
                sx={{
...
export default memo(NewEditBookmaker);
